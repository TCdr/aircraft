...
export const ApuPage = () => {
  ...
  return (
    <EcamPage name="main-apu">
      ...
      {apuAvail && (
        <SvgGroup x={305} y={85}>
          <text x={0} y={0} className="Green FontTitle Center">
            AVAIL
          </text>
        </SvgGroup>
      )}
  ...
  return (
    <>
      <SvgGroup x={x} y={y}>
        ...
        {!inModeStandby && <rect className="Box" width={100} height={111} />}
        ...
        <text
          ...
          className={`Center FontNormal
                    ${
                      ((!inModeStandby &&
                        !(apuGenPotentialNormalRange && apuGenLoadNormalRange && apuGenFreqNormalRange)) ||
                        inModeOff) &&
                      ' Amber'
                    }`}
        >
        ...
        {inModeOff && (
          <text x={50} y={70} className="Center FontNormal">
            OFF
          </text>
        )}
        ...
        {inModeOn && (
          <>
            <SvgGroup x={60} y={55}>
              {/* FBW-31-08 */}
              <text x={0} y={0} className={`Right FontLarger ${apuGenLoadNormalRange ? 'Green' : 'Amber'}`}>
                {apuGenLoad.toFixed()}
              </text>
              <text x={0} y={25} className={`Right FontLarger ${apuGenPotentialNormalRange ? 'Green' : 'Amber'}`}>
                {apuGenVoltage.toFixed()}
              </text>
              <text x={0} y={50} className={`Right FontLarger ${apuGenFreqNormalRange ? 'Green' : 'Amber'}`}>
                {apuGenFreq.toFixed()}
              </text>
            </SvgGroup>
            <SvgGroup x={70} y={55}>
              <text x={0} y={0} className="Cyan FontNormal">
                %
              </text>
              <text x={0} y={25} className="Cyan FontNormal">
                V
              </text>
              <text x={0} y={50} className="Cyan FontNormal">
                HZ
              </text>
            </SvgGroup>
          </>
  ...
  return (
    <>
      ...
      <SvgGroup x={x} y={y}>
        ...
        <text x={50} y={22} className="Center FontNormal">
          BLEED
        </text>

        <text x={44} y={48} className={`FontLarger Right ${apuBleedPressAvailable ? 'Green' : 'Amber'}`}>
          {apuBleedPressAvailable ? displayedBleedPressure : 'XX'}
        </text>
        <text x={90} y={48} className="Cyan FontNormal Right">
          PSI
        </text>

        <line className="Line Green" x1={50} y1={-1} x2={50} y2={-22} />
        <circle
          className={`Circle ${!apuBleedOpen && apuBleedPbOnConfirmed ? 'Amber' : 'Green'}`}
          cx={50}
          r={18}
          cy={-40}
        />
  ...
  return (
    <>
      <SvgGroup x={x} y={y}>
        ...
        <SvgGroup x={80} y={13}>
          <text x={0} y={0} className="White Center FontNormal">
            N
          </text>
          <text x={0} y={30} className="Cyan Center FontNormal">
            %
          </text>
        </SvgGroup>

        <text x={10} y={80} className={`FontLarger Left ${apuN.isNormalOperation() ? apuNIndicationColor : 'Amber'}`}>
    ...
    <>
      <SvgGroup x={x} y={y}>
        ...
        <SvgGroup x={80} y={13}>
          ...
          <text x={0} y={30} className="Cyan Center FontNormal" style={{ letterSpacing: -2 }}>
            &deg;C
          </text>
        </SvgGroup>

        <text x={10} y={80} className={`FontLarger Left ${egtNumericalStyle}`}>
    ...
    <>
      ...
      <SvgGroup x={x} y={y}>
        {lowFuelPressure.value && (
          <text className="Amber FontNormal" x={0} y={0}>
            FUEL LO PR
          </text>
        )}

        {apuFlapOpenPercentage === 100 && (
          <text className={`Green FontNormal ${isIntakeIndicationFlashing && 'FillPulse'}`} x={0} y={60}>
            FLAP OPEN
          </text>
        )}
        {/* FBW-31-07 */}
